...
const itemsPerpage = 4;
   ...
   useEffect(() => {
    setIsLoading(true)
    fetch(`http://localhost:3000/myJobs/suma@gmail.com`)
    .then(res => res.json())
    .then(data => {
      setJobs(data);
      setIsLoading(false);
  ...
  return (
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4' style={{ backgroundImage: `url(https://img.freepik.com/free-photo/white-paper-texture-background_23-2148171235.jpg?size=626&ext=jpg&ga=GA1.1.1865479528.1707567792&semt=sph)`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize: 'cover' }}>
...

        {
              ...
              <tbody>
            {
                currentJobs.map((job,index) => (
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <button onClick={() => handleDelete(job._id)} className='bg-blue py-2 px-6 text-white rounded-sm'>Delete</button>
                    </td>
                  </tr>
                ) )
            }
